...
const App = () => {
  ...
  const loadSchedule = (mode) => {
    ...
    // Determine the target date based on mode
    ...
    if (dayName === "Sunday") {
      ...
      if (mode === "tomorrow") {
        // TOMORROW VIEW: No top card, show ALL classes in swipe list
        setNextClass(null);
        setRemainingClasses(daySchedule);
      } else {
        // TODAY VIEW: Calculate based on current time
        processTodaySchedule(daySchedule, targetDate);
      }
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4 bg-gray-50 text-gray-800 font-sans overflow-hidden">
      ...
      <header className="w-full max-w-sm mb-6 mt-2 flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium">
            {format(displayDate, "MMMM do")}
          </p>
        </div>
        
        {/* Toggle Buttons */}
        <div className="bg-white p-1 rounded-xl shadow-sm border border-gray-200 flex text-xs font-bold">
             <button 
      ...
      {/* TOP CARD SECTION */}
      {/* Logic: Only show if viewMode is 'today'. If it's Sunday 'today', show Sunday card. If 'tomorrow', hide completely. */}
      {viewMode === "today" && (
        <div className="w-full max-w-sm mb-8 z-10 animate-fade-in-down">
            ...
            <div className="bg-gradient-to-br from-green-400 to-emerald-600 rounded-3xl shadow-2xl p-6 text-white transform transition hover:scale-[1.02]">
                ...
                No classes today. Monday's schedule is prepared below.
                </p>
            </div>
            ) : nextClass ? (
            // NEXT CLASS CARD
